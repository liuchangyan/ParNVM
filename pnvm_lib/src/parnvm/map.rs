    ...
    V: Debug,
{
    ...
    pub fn get(&self, k: &K) -> Option<Accessor<K, PValue<V>>> {
        self.inner_.find(k)
    }
    ...
    pub fn insert(&self, key: K, pval: PValue<V>) -> Option<PValue<V>> {
    ...
    V: Debug,
{
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            f,
            "PValue {{ data: {:?}, last_writer_: {:?}, write_locked: {:?} }}",
            self.data_,
            self.last_writer_.get(),
            self.is_write_locked_
        )
    }
}
