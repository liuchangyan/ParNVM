...
pub fn do_pc_new_order(tables: &Arc<Tables>, tx: &mut TransactionParOCCRaw, input: NewOrderInput)
    -> bool
{ 
    ...
    while {
        tables.neworder.push_pc_raw(tx,
    ...
    while {
        ...
        for i in 0..ol_cnt as usize {
            ...
            tables.orderline.push_pc_raw(tx, 
                                     OrderLine {
...
pub fn do_pc_stocklevel(tables: &Arc<Tables>, w_id:i32, d_id:i32, thd:&Numeric, tx: &mut TransactionParOCCRaw)
    -> bool
{
...
pub fn do_pc_delivery(tables: &Arc<Tables>, w_id: i32, o_carrier_id: i32, tx: &mut TransactionParOCCRaw) 
    -> bool
{
    tx.activate_txn();
    let num_dis = num_district_get();
    ...
    while {
        for d_id in 1..=num_dis {
            ...
            if no_arc.is_some() {
                ...
                if !tables.neworder.delete_pc_raw(tx, &(w_id, d_id, no_o_id), &tables) {
                    return false;
                }
    ...
    -> OrderStatusInput
{
    ...
    let d_id = urand(1, 10, rng); let w_id = w_home;
...
pub fn do_pc_orderstatus(tables: &Arc<Tables>,
