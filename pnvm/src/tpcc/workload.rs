...
fn new_order(tx: &mut TransactionOCC, 
             ...
             now: i64)
{
    let w_row = tables.warehouse.retrieve(&w_id).unwrap();
    let w_tax = tables.warehouse.retrieve(&w_id).unwrap().read(tx).w_tax;
    let c_discount = tables.customer.retrieve((w_id, d_id, c_id)).unwrap().read(tx).c_discount;
    let district_ref = tables.district.retrieve((w_id, d_id)).unwrap();
